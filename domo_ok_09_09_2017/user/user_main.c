...
#ifdef PWM
char BIT_PWM[NB_PWM]={BIT_PWM_TEST};
#endif
...
volatile unsigned char TLI4970_is_enable=1;
#endif
...
#ifdef PWM
volatile unsigned int puissance_pwm[NB_PWM]={5};
#endif
...
bool ICACHE_FLASH_ATTR config_ap()
{
	...
	wifi_set_opmode(0x01); // Set station +ap mode
...
{
	//timer_change(10000-100,(t_call_back)timerEnableTriack);
...
void ZeroCrossIRQ()
{
	//if(temp_from_zero_cross>30)
	{
		...
		GPIO_REG_WRITE(GPIO_STATUS_W1TC_ADDRESS, BIT(RECEIVEGPIO_ZERO_CROSS));//ici clear irq
...
#else
	GPIO_REG_WRITE(GPIO_STATUS_W1TC_ADDRESS, BIT(RECEIVEGPIO_IRQ_PCF8885));//ici clear irq
#endif
}
...
{
	ETS_GPIO_INTR_DISABLE();	
	ETS_GPIO_INTR_ATTACH(pcf8885func, RECEIVEGPIO_IRQ_PCF8885);
